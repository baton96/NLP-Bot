...
def measure_by(tmp_df, by=None, metric='count', top=-1):
    if by == 'year':
...
@show_or_return
def plot(by = 'title', kind="bar", top=-1, no_groups_arg=False, only_groups_arg=False, received_arg=False, sent_arg=False):
    tmp_df = df.copy()
    if no_groups_arg:
    ...
    if kind == "pie":
        ...
        graph = go.Bar(x=tmp_df[by], y=tmp_df.metric, name='')
        fig = go.Figure(graph)
...
when = make_subplots(
    ...
    subplot_titles=['Rok', 'Miesiąc', 'Dzień tygodnia', 'Godzina']
)
when.add_trace(plot(by='year')[1], row=1, col=1)
when.add_trace(plot(by='month')[1], row=1, col=2)
when.add_trace(plot(by='weekday')[1], row=2, col=1)
when.add_trace(plot(by='hour')[1], row=2, col=2)
